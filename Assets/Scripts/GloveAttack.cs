...
public class GloveAttack : WeaponAttack
{
    ...
    {
        cur_AttackPower = 1;
        StartCoroutine(Sub_Glove(num));
        return Weapon.SubWeaponCooltime - ReduceSubWeaponCooltime;
    }
    IEnumerator Sub_Glove(int num)
    {
        ...
        * BonusDamage;
        ...
        temp.transform.position += Vector3.up * 0.1f;
        temp.transform.localScale *= PlayerStatus.Instance.Value_AttackRange;
        temp.GetComponent<GloveSkillObject>().FinalDamage = dmg;
